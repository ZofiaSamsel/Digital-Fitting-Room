...
class FashionPose():
    ...
    def draw_torso(self, image, img_torso_keypoints):
        ...
        original_torso_keypoints = np.array([[0, 0], [self.torso_image.shape[1] - 1, 0],
                                             [self.torso_image.shape[1] // 2, self.torso_image.shape[0] - 1]],
                                            dtype=np.float32)
    ...
    def draw_bone(self, image, bone_keypoints):
        ...
        original_bone_keypoints = np.array(
            [[self.bone_image.shape[1] // 2, 0], [self.bone_image.shape[1] // 2, self.bone_image.shape[0] - 1]],
            dtype=np.float32)
...
def select_skull_landmark_pixels():
    ...
    cv2.namedWindow("fashion", cv2.WINDOW_NORMAL)
    cv2.setMouseCallback("fashion", draw_mouseclick_circle)
    while (1):
        cv2.imshow("fashion", skull_image)
